...
//  var ajax_url = "http://sharekhan.noesis.tech/rest";
var beta_thankyou_url = "https://noesis.tech/demos/projectleapp/phase_1/betaThankyou.html";
var beta_thankyou_url = "https://noesis.tech/demos/projectleapp/phase_1/betaThankyou.html";
var alpha_thankyou_url = "https://noesis.tech/demos/projectleapp/phase_1/thankyou.html";
var alpha_thankyou_url = "https://noesis.tech/demos/projectleapp/phase_1/thankyou.html";
var registration_url = "https://noesis.tech/demos/projectleapp/phase_1/register.html";
var lead_form_url = "https://noesis.tech/demos/projectleapp/phase_1/";


// var beta_thankyou_url = "betaThankyou.html";
// var beta_thankyou_url = "betaThankyou.html";
// var alpha_thankyou_url = "http://foxybrain.foxymoron.tv/sharekhan/thankyou.html";
// var alpha_thankyou_url = "http://foxybrain.foxymoron.tv/sharekhan/thankyou.html";
...
const onRadioOptionSelection = ans_obj => {
  ...
  for (let i = 0; i < length_radio.length; i++) {
    ...
    if ($radio != null) {
      ...
      var radio_option_checked = respdata[ans_obj.current_section][ans_obj.current_index].select_answer.split("_").pop();

      var objCount = respdata[ans_obj.current_section][ans_obj.current_index].options.length;
      for (let x = 0; x < objCount ; x++) {
          if(respdata[ans_obj.current_section][ans_obj.current_index].options[x].value == radio_option_checked){
            var radio_curitem = respdata[ans_obj.current_section][ans_obj.current_index].options[x].label;
            var radio_currquestion = respdata[ans_obj.current_section][ans_obj.current_index].question;
          }
      }
...
const onCheckboxSelection = ans_obj => {
  ...
  for (let i = 0; i < selected_answer.length; i++) {
    ...
    var checkbox_option_checked = optionSelected.split("_").pop();

    var objCount = respdata[ans_obj.current_section][ans_obj.current_index].options.length;
    for (let x = 0; x < objCount ; x++) {
        if(respdata[ans_obj.current_section][ans_obj.current_index].options[x].value == checkbox_option_checked){
          var checkbox_curitem = respdata[ans_obj.current_section][ans_obj.current_index].options[x].label;
          checkbox_arr.push(checkbox_curitem);
        }
    }

    var chkbox_currquestion = respdata[ans_obj.current_section][ans_obj.current_index].question;
  }
...
const onTextareaEnter = ans_obj => {
 ...
 var textarea_currquestion = respdata[ans_obj.current_section][ans_obj.current_index].question;

  // dtm_datalayer.push({
  //     'event': 'beta_question',
  //     'question': textarea_currquestion,
...
$(document).ready(function() {
  $.ajax({
    ...
    success: function(data, textStatus, jqXHR) {
      ...
      if (count_val.count > 800) {
        var fbTrack = document.createElement("script");
        var dom = `fbq('track', 'Lead');`;
        fbTrack.innerHTML = dom;
        // fbTrack.setAttribute('id','lead');
        $("head").append(fbTrack);
