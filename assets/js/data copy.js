         // Validate forms
            ...
            $('.registration-form').on('click','.reg_form_next',function(event){
                ...
                if(event.currentTarget.name === "next1") {
                    ...
                    if(respdata[ans_obj.current_section].section_1_attempted){
                        ...
                        current_step = $(this).parent().parent().parent().next();
                        current_step.removeClass('transitBack').addClass('active_question');
                        prev_step = $(this).parent().parent().prev();
                        ...
                        let threshold = 6.667;
                        currentWidth < 100 && $('#progressBar').css('width', currentWidth + threshold + '%');
                    ...
                    if(respdata[ans_obj.current_section].section_1_attempted && checkStatus){
                        sendData(respdata);
                        window.location.href = "http://tiger.foxymoron.tv/sharekhan/thankyou.html";
                    }
                }
                else {
                        ...
                        if(respdata[ans_obj.current_section][ans_obj.current_index].user_attempted || 
                           respdata[ans_obj.current_section][ans_obj.current_index].select_answer == null ||
                            ...
                            if(ans_obj.current_section == "traderorinvestor"){
                                var currentWidth = parseFloat($('#progressBar').css('width')) / 2.95;
                            ...
                            if($('input[name=input_9]:checked').length > 0){
                                var searchIDs = $("input[name=input_9]:checked").map(function(){
                                    return $(this).attr('id');
                                }).get();
                            ...
                            if(input10kapassed || respdata[ans_obj.current_section][ans_obj.current_index].select_answer != "option_10k"){
                                question_no += 1;
                                $('.question_no').html('<span class='+question_no+'>Question-'+question_no+'</span>');
                                var next_question = loadNextQuestion(ans_obj);
                                current_section = next_question.current_section;
                                current_index = next_question.current_index;
                            }
          ...
          function validateRecaptcha() {
                ...
                if (ans_obj.element.closest('#form_01').find("input").length>0){
                    //Save multiple answers
                        ...
                        for (var x=0; x<length; x++){
                            var resp_index = respdata[ans_obj.current_section][ans_obj.current_index].map(function(o){return o.key}).indexOf(ans_set[x].name);
                            respdata[ans_obj.current_section][ans_obj.current_index][resp_index].user_answer = ans_set[x].value;
                            if(respdata[ans_obj.current_section][ans_obj.current_index][resp_index].user_answer == ''){
                                null_value += 1;
                            }
                            if(respdata[ans_obj.current_section][ans_obj.current_index][resp_index].user_answer != ''){
            ...
            const loadNextQuestion = (ans_obj) => {
                ...
                if((typeof(section_question_count)=="undefined") || (section_question_count - 1) == ans_obj.current_index){ 
                    if (ans_obj.next=='calculate_next' ){
                        if (conditonal_questions_complete == false){
                            if(option_7D_selected && question_7_completed == false){
                                ...
                                window.location.href = "http://tiger.foxymoron.tv/sharekhan/thankyou.html";
                                // window.location.href = "http://localhost/sharekhan_html/thankyou.html";
                            }
                    ...
                    else if(ans_obj.current_section == "traderorinvestor" && respdata[ans_obj.current_section][ans_obj.current_index].select_answer != "option_7a" &&
                    respdata[ans_obj.current_section][ans_obj.current_index].id == 7) {
                        ...
                        if(ans_obj.current_section == "traderorinvestor" && 
                            respdata[ans_obj.current_section][ans_obj.current_index].select_answer == "option_7d" &&
                            respdata[ans_obj.current_section][ans_obj.current_index].id == 7){
                            option_7D_selected = true;
                        }
                        else{
                        ...
                        response.current_section = ans_obj.current_section;
                        console.log("next question",respdata[response.current_section][response.current_index]);
                        //if next question is the last one, then we should add next parameter
                        console.log(respdata[response.current_section].length,(response.current_index+1));
                        if (respdata[response.current_section].length == (response.current_index+1)){
                            ...
                            if( response.current_section == "approachesToTrading_GRP1"  ||
                                response.current_section == "approachesToTrading_GRP2" ||
                                response.current_section == "approachesToTrading_GRP3" ||
                                response.current_section == "all_sections" ||
                                response.current_section == "future_group" ||
                                ...
                                response.current_section == "investment_group")
                                {
                                    ...
                                    if($('input[name=tandc_block]:checked').length > 1){
                                        $('.next2').attr('disabled', false).removeClass('disable-hover');
                                    }
                                    else{
                                        $('.next2').attr('disabled', true).addClass('disable-hover');
                    ...
                    var template = jQuery("#"+respdata[response.current_section][response.current_index].question_type).html();
                    respdata[response.current_section][response.current_index].current_section=response.current_section;
                    respdata[response.current_section][response.current_index].current_index=response.current_index;
                    respdata[response.current_section][response.current_index].next=response.next;
                    var render = Mustache.render(template,respdata[response.current_section][response.current_index]);

                    var question = document.createElement('div');
                    question.setAttribute('id', 'question_'+respdata[response.current_section][response.current_index].id);
                    ...
                    for(let i = 0; i < arrayLength; i++){
                        var childId = $('.demo-form').children()[i].getAttribute('id');
                    ...
                    if(event_click_next2){
                        current_step = $('#'+ans_obj.data.input_id).parent().parent();
                        next_step = $('#'+ans_obj.data.input_id).parent().parent().next();
                        var arr = [];
                        $.each(arr, function(e) { 
                            arr.push(next_step);
                        });
                    ...
                    let skipdiff = respdata[response.current_section][response.current_index].id - ans_obj.data.input_id;
                    ...
                    setTimeout(() => {
                        $(`#question_${respdata[response.current_section][response.current_index].id - skipdiff}`).addClass('transitOut');  
                        setTimeout(() => {     
            ...
            function setProgressBar(ans_obj, count){
                ...
                if(!progressBarReset){
                    percent = parseFloat(100 / section_count) *(section_count -  Math.abs(count));
            ...
            const onRadioOptionSelection = (ans_obj) => {             
                // var $radio = $('input:radio.form_data_radio:checked');
                    ...
                    let length_radio = $('input:radio[name=input_'+respdata[ans_obj.current_section][ans_obj.current_index].id+']');
                    for(let i = 0; i < length_radio.length; i++){
                        var $radio = $('input:radio[id=option_'+respdata[ans_obj.current_section][ans_obj.current_index].options[i].value+']:checked');
                        console.log($radio);
                        if($radio != null){
                                ...
                                if(selected_answer == "option_10k" && respdata[ans_obj.current_section][ans_obj.current_index].id == 10){
                                    ...
                                    for (let i = 0; i < textLength; i++){
                                        ...
                                        if(option_10ka_value != ""){
                                            respdata[ans_obj.current_section][ans_obj.current_index].textBox_input_value = option_10ka_value;
                                        }
                                    }
                                }
                            }
            ...
            const onCheckboxSelection = (ans_obj) => {
                var $checkbox =  $('input:checkbox[name=input_'+respdata[ans_obj.current_section][ans_obj.current_index].id+']:checked');
                var arr = [];
            ...
            function checkInput9(selected_answer){
                for(let i = 0; i < selected_answer.length; i++){
                    ...
                    else if(optionSelected == "option_9b" || optionSelected == "option_9c" 
                            || optionSelected == "option_9d" || optionSelected == "option_9e"){
                        group2 = true;
                    }
            ...
            $("#registrationForm").on('click', '.reg_form_previous, #reg_form_previous', function (e) {
            ...
            const loadPreviousQuestion = (ans_obj) => {
                ...
                if(response.current_section == "traderorinvestor" && response.current_index == 0){
                    current_step = $('.reg_form_previous, #reg_form_previous').parent().parent();
                    prev_step = $('.reg_form_previous, #reg_form_previous').parent().parent().prev();
                    ...
                    //if($('input[name=input_7]').is(':checked') && respdata[response.current_section][1].select_answer != "option_7a" && ans_obj.data.input_id == 9)
                    //pushpa
                    if(ans_obj.data.input_id == 9){
                        response.current_index = ans_obj.current_index - 2;

                    }
                    else if((response.current_section == "approachesToTrading_GRP1" ||
                            response.current_section == "approachesToTrading_GRP2" ||
                            response.current_section == "approachesToTrading_GRP3" ||
                            response.current_section == "all_sections" || 
                            response.current_section == "future_group" ||
                            response.current_section == "options_group" ||
                            response.current_section == "investment_group") && ans_obj.current_index == 0){
                            
                            if($('input[id=option_7d]').is(':checked')  && ans_obj.current_index == 0){
                                response.current_section = "dependsOn_7d";
                                response.current_index = 4;
                            }
                            else if($('input[id=option_7d]').is(':checked')  && ans_obj.current_index > 0){
                                response.current_index = ans_obj.current_index - 1;
                            }
                            else{
                    ...
                    else if(response.current_section == "dependsOn_7d" && ans_obj.current_index == 0) {
                            response.current_section = "traderorinvestor";
                            response.current_index = 5;
                    }
                    else{
                    ...
                    let skipdiff = ans_obj.data.input_id - respdata[response.current_section][response.current_index].id; 
                    setTimeout(() => {
                        $(`#question_${ans_obj.data.input_id}`).addClass('transitBack').hide();
                        setTimeout(() => {     
                            $(`#question_${ans_obj.data.input_id - skipdiff}`).removeClass('transitOut').addClass('active_question');
                        }, 300);
                    }, 100);
                    // setTimeout(() => {
                    //     if(respdata[response.current_section][response.current_index].id == 10){
                    ...
                    if(ans_obj.current_section == "traderorinvestor"){
                        ...
                        var current_section_length = respdata[ans_obj.current_section].length;
                        let currentWidth = parseFloat($('#progressBar').css('width')) / current_section_length;
                        let threshold = 6.6667;
                        $('#progressBar').css('width', currentWidth + threshold + '%');
                    }
...
function sendData(data){
    ...
    // }
      $.ajax({
        url: 'http://sharekhan.noesis.tech/rest',
        type: 'POST',
  ...
  function sendOtp(){
    ...
    $("#otp_submit").click(function(){
          ...
          if(otpVerified){
            ...
            $('.otp_submit').css('margin-top','2rem');
          }
    });
